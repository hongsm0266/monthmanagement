...
# 🚨 신규 인원 소속 완벽 반영 (이곳에 누락된 영업사원 추가 시 전체 합계 자동 반영)
HC_DB = {
...
if not st.session_state['logged_in']:
    ...
    with col_center:
        ...
        if st.button("로그인", use_container_width=True):
            ...
            elif pad_id in HC_DB and str(login_id).strip() == input_pw: 
...
# 🚀 [범위 B10:AG200 으로 상향 확대] (위로 올라간 인원 방어)
@st.cache_data(ttl=60) 
def load_perf_sheet(_gc_client):
    try:
        data = _gc_client.open(SHEET_NAME).worksheet("시트1").get("B10:AG200")
        if data:
...
# 🚀 [VDT 지표 매핑 최적화 및 W열(전월마감) 반영]
# F(4)=견적_일, G(5)=계약_일, H(6)=견적_누적, I(7)=계약_누적, R(16)=계약금액_누적
# T(18)=당월매출, U(19)=전월매출, W(21)=전월마감, Y(23)=익월매출
def get_perf_metrics(perf_df, target_id, target_name):
    ...
    if target_id == "ALL":
        ...
        # 🚀 [보강 2] 사번이 00042008 이든 42008 이든, 사업자가 나뉘어 여러 줄이든 모두 한 사람으로 인식해서 합산!
        possible_ids = [
            str(target_id), 
...
R_str, Y_str, W_str = fmt(metrics['R']), fmt(metrics['Y']), fmt(metrics['W']) # 🚀 전월마감 W열 추가!
