...
@st.cache_resource
def init_connection():import socket
socket.setdefaulttimeout(15.0) 
...
def load_vdt_data():
    ...
    try:
        ...
        # 1. A16:D48 범위에서 인별 매출목표 및 비중 계산
        status_box.info("📊 1단계: 인별 매출목표 및 비중 계산 중...")
        hc_target_data = target_ws.get("A16:D48")
...
if not df_raw.empty:
    ...
    def render_custom_html_table(df):
        ...
        html.append("<tr><th rowspan='3'>대리점</th><th rowspan='3'>HC명</th>")
        
        headers_l1 = []
        for col in df.columns[2:]:
            if col[0] not in headers_l1:
                headers_l1.append(col[0])
                
        for h1 in headers_l1:
            is_curr = CURRENT_WEEK in h1
            bg_style = "style='background-color:#1e3a8a; border-left:2px solid #3b82f6; border-right:2px solid #3b82f6;'" if is_curr else ""
            
            if "🎯" in h1:
                html.append(f"<th colspan='3' {bg_style}>{h1}</th>")
            else:
                html.append(f"<th colspan='9' {bg_style}>{h1}</th>")
        html.append("</tr>")
        
        html.append("<tr>")
        for h1 in headers_l1:
            is_curr = CURRENT_WEEK in h1
            bg_style = "style='background-color:#1e293b; border-left:2px solid #3b82f6; border-right:2px solid #3b82f6;'" if is_curr else ""
            
            if "🎯" in h1:
                html.append(f"<th colspan='3' {bg_style}>인별매출(천)</th>")
            else:
                html.append(f"<th colspan='3' {bg_style}>계약액(천)</th>")
                html.append(f"<th colspan='3' {bg_style}>견적건</th>")
                html.append(f"<th colspan='3' {bg_style}>계약건</th>")
        ...
        for col in df.columns[2:]:
            is_curr = CURRENT_WEEK in col[0]
            bg_style = "style='background-color:#334155; border-left:2px solid #3b82f6; border-right:2px solid #3b82f6;'" if is_curr else ""
            html.append(f"<th {bg_style}>{col[2]}</th>")
        html.append("</tr></thead><tbody>")
        ...
        for r_idx in range(len(df)):
            ...
            html.append(f"<tr {row_bg}>")
            html.append(f"<td class='text-center'>{dealer}</td>")
            html.append(f"<td class='text-center'>{hc}</td>")
            
            for c_idx, val in enumerate(row[2:]):
                col_obj = df.columns[c_idx + 2]
                is_curr = CURRENT_WEEK in col_obj[0]
                border_style = "border-left:2px solid #3b82f6; border-right:2px solid #3b82f6;" if is_curr else ""
    ...
    st.info(f"💡 현재 진행 주차: **'{CURRENT_WEEK}'** (주차별 파란 테두리 및 대리점별 구분 음영 적용 완료)")
